...
/** Returns the lowercase SHA-256 digest without depending on a platform cryptography provider. */
internal fun ByteArray.sha256Hex(): String = Sha256Digest.digest(this)
    .joinToString(separator = "") { byte -> HEX[(byte.toInt() ushr 4) and 0x0f].toString() + HEX[byte.toInt() and 0x0f] }

private object Sha256Digest {
    fun digest(input: ByteArray): ByteArray {
        val padded = input.padded()
        ...
        while (offset < padded.size) {
            ...
            for (index in 0 until 64) {
                ...
                val temporary1 = h + sum1 + choice + ROUND[index] + schedule[index]
    ...
    private fun ByteArray.padded(): ByteArray {
        val zeroBytes = (56 - ((size + 1) % 64) + 64) % 64
        val result = ByteArray(size + 1 + zeroBytes + 8)
        copyInto(result)
        result[size] = 0x80.toByte()
    ...
    private val INITIAL: IntArray = intArrayOf(
        0x6a09e667,
        0xbb67ae85.toInt(),
        0x3c6ef372,
        0xa54ff53a.toInt(),
    ...
    private val ROUND: IntArray = intArrayOf(
